...
时间复杂度为`O(nlogn)`的排序算法有：快速排序，堆排序和归并排序，**最适合链表这一数据结构的是归并排序**

1. 快慢指针寻找链表中间节点时注意事项
	1. **快指针需要先走一步**，不然在2节点下，中间节点始终指向第二个，无法切开
2. 递归时弄清楚递归函数的含义，返回值的含义是什么
	1. 官方题解中：
		1. `sortFunc`的返回值就是head到tail的有序链表
		2. merge的含义就是合并两个有序链表，并返回合并链表的头指针
...
1. 怎么样会导致重名呢？
	1. 两名称首字母相同
	2. 首字母不同，但剩余部分相同
2. 创建一个hashmap，以首字母为key值，value为以该首字母开头的所有后缀（set）
3. hashmap中的key两两成对，并取value的交集（后缀相同），可成功的搭配为：`(len(a)-m)*(len(b)-m)`

### [146. LRU 缓存](https://leetcode.cn/problems/lru-cache/)

存储最久未被访问的，想到队列
但由于`get`的时间复杂度为O(1)，所以要使用hashmap，两者结合一下即可

问题：get时需要更新最新访问的元素，此时用队列做不到O(1)，看了提示之后可以使用双向链表实现，lru表示最久未使用的节点，同时使用hashmap存储每个key对应的节点，访问时可以直接定位到对应节点更新。
...
最直观的思路是利用hashmap存储已经访问的节点，然后遍历时判断下一个是否在hashmap中即可

题解：
1. 快慢指针，假设链表环前长度为k，环长为m，最后两指针相遇的位置距环开端x
2. 慢指针走k+?m+x，两者相遇，快指针肯定比慢指针多走了m的倍数，故`k+?m+x=?m`，则`x=?m-k`
3. 此时一个新指针从头开始走，其一定和慢指针在环开端相遇
...
递归：
1. 递归终止条件：s为空，则为True
2. 给定字符串s，其是否可由给定字典构成等价于`s[i:]，s[i+r:]...`能否由给定字典构成
	1. 其中`s[:i],s[:i+r]...`存在于字典中

在第35个样例中超时，函数前加上`@cache`装饰器便不会超时了...
### [2286. 以组为单位订音乐会的门票](https://leetcode.cn/problems/booking-concert-tickets-in-groups/)

这道题的题目很绕，灵神给了易于理解的题意：
...
之前有道题：多数元素，看似很想，但实际没啥关系

由于此处找只出现一次，其余均出现两次，利用异或相同元素相消的逻辑可以解决
...
`dp[i][j]`表示以i开头，j结尾的子串是否为回文串
建立递推公式的时候，需要注意建立`dp[i][j]`怎么推导，而不是怎么根据其推导其他状态
