...
利用`ping`命令发出100个ICMP报文，统计VNI模块发送和接收的分组个数，每分钟定时打印以下信息：
1. 发送端发送分组总数，每分钟的发送速率
2. 接收端的接受分组总数，每分钟的发送速率

由于在IP层和链路层之间添加了`VNI`头，故`ICMP`数据包的传输过程必须单独编写代码控制，不能直接交给内核处理。

内核态程序：
1. VNI模块需要捕获（通过`netfilter`预留的hook）IP层发往数据链路层的数据包，为其添加`VNI`包头，然后将其通过无线网口输出。
2. 统计分组收发速率也在内核态中完成，注册定时器，将统计信息发送到用户态程序输出。

用户态程序：
1. 通过`libpcap`捕获数据链路层的`vni`数据包，然后通过`netlink`发送给内核，也即`vni`数据包的生命周期交给我们编写的代码控制
2. 接收内核发送的分组统计信息，计算分组收发速率并打印输出
